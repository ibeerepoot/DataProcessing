...
def scrape_movie_page(dom):
    ...
    # scrape title of movie
    title = dom.by_class('header')[0].children[1].content

    # scrape duration of movie
    tempduration = dom('.infobar time')[0].content
    # delete the last part: " min" and the spaces in front of it
    duration = tempduration[:-4].strip()

    # scrape genres of movie
    tempgenres = ""
    genres = ""
    genrecounter = 0
    for numgenres in dom('.infobar > a'): 
        # add the genre to the string + a semicolon and space
        tempgenres += dom('span[itemprop="genre"]')[genrecounter].content + "; "
        genrecounter += 1
    # remove the last two characters in the string: the semicolon and space
    genres = tempgenres[:-2]

    # scrape directors of movie
    tempdirectors = ""
    directors = ""
    directorcounter = 0
    for numdirectors in dom('div[itemprop="director"] a span[itemprop="name"]'): 
        # add the genre to the string + a semicolon and space
        tempdirectors += dom('div[itemprop="director"] a span[itemprop="name"]')[directorcounter].content + "; "
        directorcounter += 1
    # remove the last two characters in the string: the semicolon and space
    directors = tempdirectors[:-2]

    # scrape writers of movie
    tempwriters = ""
    writers = ""
    writerscounter = 0
    for numwriters in dom('div[itemprop="creator"] > a span[itemprop="name"]'): 
        # add the genre to the string + a semicolon and space
        tempwriters += dom('div[itemprop="creator"] > a span[itemprop="name"]')[writerscounter].content + "; "
        writerscounter += 1
    # remove the last two characters in the string: the semicolon and space
    writers = tempwriters[:-2]

    # scrape actors of movie
    tempactors = ""
    actors = ""
    actorscounter = 0
    for numactors in dom('div[itemprop="actors"] > a span[itemprop="name"]'): 
        # add the genre to the string + a semicolon and space
        tempactors += dom('div[itemprop="actors"] > a span[itemprop="name"]')[actorscounter].content + "; "
        actorscounter += 1
    # remove the last two characters in the string: the semicolon and space
    actors = tempactors[:-2]

    # scrape rating of movie
    rating = dom('span[itemprop="ratingValue"]')[0].content

    # scrape numratings of movie
    n_ratings = dom('span[itemprop="ratingCount"]')[0].content
